...
namespace osu.Game.Graphics.UserInterface
{
    public partial class BarGraphFlow : FillFlowContainer<Bar>
    {
        ...
        public new BarDirection Direction
        {
            ...
            {
                direction = value;
                base.Direction = direction.HasFlagFast(BarDirection.LeftToRight) || direction.HasFlagFast(BarDirection.RightToLeft) ? FillDirection.Vertical : FillDirection.Horizontal;
        ...
        public IEnumerable<float> Values
        {
            set
            {
                ...
                RemoveRange(Children.Where((bar, index) => index >= value.Count()).ToList(), true);

                updateBarsLayout();
            }
        }

        public BarGraphFlow(Action<(Bar bar, int index, int count)>? parameters = null)
        {
        ...
        private void updateBarsLayout()
        {
            ...
            float relativeSpacing = direction.HasFlagFast(BarDirection.LeftToRight) || direction.HasFlagFast(BarDirection.RightToLeft) ? (Spacing.Y / DrawHeight) : (Spacing.X / DrawWidth);
            ...
            for (int i = 0; i < Children.Count; i++)
            {
                ...
                bar.Size = direction.HasFlagFast(BarDirection.LeftToRight) || direction.HasFlagFast(BarDirection.RightToLeft) ? new Vector2(1, size) : new Vector2(size, 1);
